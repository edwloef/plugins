...
impl Cooler {
	pub const ID: &'static str = "com.edwloef.cooler";
}

impl Plugin for Cooler {
...
impl DefaultPluginFactory for Cooler {
	fn get_descriptor() -> PluginDescriptor {
		PluginDescriptor::new(Self::ID, "Cooler")
	...
	fn process(
		...
		for batch in events.input.batch() {
			...
			let process = |input: f32| {
				(input * pregain) * (1.0 - intensity) / (1.0 - intensity * (input * pregain).abs())
					* postgain
			};
